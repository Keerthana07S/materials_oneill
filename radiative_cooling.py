...
df = pd.read_csv("radiative_cooling.csv")
df['stability'] = df['stability'].apply(lambda x: 1 if x == True else 0)

properties = df[["band gap", "stability"]].values
...
required_properties = np.array([6.5, 1.0])
required_properties_normalized = (required_properties - properties_mean) / properties_std

weights = np.array([1.0, 1.0])  # Higher weight for band gap & anisotropy
...
def objective_function(weights):
    composite = composite_properties(weights, closest_materials[["band gap", "stability"]].values)
...
final_properties = composite_properties(optimized_weights, closest_materials[["band gap", "stability"]].values)

final_properties_original = final_properties * properties_std + properties_mean  

print(f"\nFinal composite properties:")
print(f'Band Gap: {final_properties_original[0]:.4f}')
print(f'Stability: {final_properties_original[1]:.4f}')
